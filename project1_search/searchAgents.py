...
def cornersHeuristic(state, problem):
    ...
    xlen = corners[3][0]
    ylen = corners[3][1]
    if len(state[1]) == 0: return 0  # Default to trivial solution
    #####score 2/3######
    h = xlen+ylen-2
    for i in state[1]:
        tmp_h = abs(i[0] - state[0][0]) + abs(i[1] - state[0][1])
        if (tmp_h < h):
            h = tmp_h
    #####score 3/3######
    # h = 0
...
def foodHeuristic(state, problem):
    ...
    h = 0
    # for food in foodLst:
    #     tmp_h = abs(food[0] - position[0]) + abs(food[1] - position[1])
    #     if (tmp_h > h):
    #         h = tmp_h
    ####score 5/4####
    foodLst = foodGrid.asList()
    ...
    h = 0
    for food in foodLst:
        tmp_h = mazeDistance(position, food, problem.startingGameState)
        if (tmp_h > h):
            h = tmp_h
    return h
